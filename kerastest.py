...
for pic in Path().glob(r'pic/[a-zA-Z]*.png'):
    ...
    inp.append([[[z] for z in x] for x in (255 - cv2.imread(str(pic), 0)) / 255])
...
model.add(Conv2D(8, kernel, activation='relu', padding='same', input_shape=(16, 16, 1)))
...
model.fit(inp, char, epochs=150, batch_size=32)
